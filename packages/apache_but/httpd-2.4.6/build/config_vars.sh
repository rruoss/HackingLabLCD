...
if test -f ${DESTDIR}/opt/apache_but/httpd//bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/opt/apache_but/httpd//bin/apr-1-config
   APU_CONFIG=${DESTDIR}/opt/apache_but/httpd//bin/apu-1-config
else
...
test -n "/opt/apache_but/httpd//bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"
