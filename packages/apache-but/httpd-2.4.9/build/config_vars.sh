...
if test -f ${DESTDIR}/opt/apache_but/apr-1.5.0/bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/opt/apache_but/apr-1.5.0/bin/apr-1-config
   APU_CONFIG=${DESTDIR}/opt/apache_but/apr-util-1.5.3/bin/apu-1-config
else
...
test -n "/opt/apache_but/apr-util-1.5.3/bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/opt/apache_but/httpd//build"
