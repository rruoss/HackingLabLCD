...
if(OSNAME >!< "none"){
  ...
  if(gnomescreensaver >< "error"){
    ...
    if (sshlog) desc = string("Beim Testen des Systems trat ein Fehler auf: " + sshlog);
     ...
     if (screensaverdaemon == "none") desc += string('\nDer Schlüsselname /apps/gnome_settings_daemon/screensaver/start_screensaver wurde nicht gefunden.');
  }else if(gnomescreensaver == "true" && screensaverdaemon == "false"){
     result = string("nicht erfüllt");
     desc = string('Es wurde die Bildschirmsperre mit Passwortschutz aktiviert.\nAllerdings steht der Schlüsselname /apps/gnome_settings_daemon/screensaver/start_screensaver auf false.\nSetzen Sie diesen Wert auf »True«, um den Bildschirmschoner beim Anmelden zu starten.');
  }else {
     result = string("nicht erfüllt");
  ...
  if(defkdescreensav >< "error"){
    ...
    if (sshlog) desc = string("Beim Testen des Systems trat ein Fehler auf: " + sshlog);
     ...
     result = string("unvollständig");
     desc = string('Es wurde die Bildschirmsperre mit Passwortschutz bei allen Usern aktiviert.\nAllerdings wurde die Bildschirmsperre mit Passwortschutz in der Datei /etc/kde4/share/config/kscreensaverrc nicht richtig konfiguriert.');
  }else if(defkdescreensav == "true" && userkdescreensav == "false"){
     result = string("nicht erfüllt");
     desc = string("Es gibt User bei denen die Bildschirmsperre mit Passwortschutz nicht aktiviert ist.");
  }
}else{
  result = string("Fehler");
  desc = string('Anscheinend handelt es sich bei dem System weder um ein Microsoft Windows, ein GNOME oder ein KDE System.\nZumindest konnte die Konfiguration für  Bildschirmsperre mit Passwortschutz nicht erkannt werden.');
}
if (!result){
  result = string("Fehler");
