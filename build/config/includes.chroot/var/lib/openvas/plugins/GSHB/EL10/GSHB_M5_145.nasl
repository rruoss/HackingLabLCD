...
if(OSNAME >!< "none"){
  ...
  result = string("nicht zutreffend");
  desc = string('Weder CUPS noch der CPUS-Client konnten auf dem System gefunden werden.');
}else{
  ...
  if (cupsd >< "nocupsd" && cupsclient >!< "nocupsclient"){
    if (cupsclient >< "no client.conf"){
      ...
      if(ServerName >< "none" || Encryption >< "none"){
        result = string("nicht erfüllt");
        if(ServerName >< "none" && Encryption >!< "none")desc = string('Beim Testen des Systems wurde in der Datei /etc/cups/client.conf, der Eintrag -ServerName- nicht gefunden.');
        if(ServerName >!< "none" && Encryption >< "none")desc = string('Beim Testen des Systems wurde in der Datei /etc/cups/client.conf, der Eintrag -Encryption- nicht gefunden.');
        if(ServerName >< "none" && Encryption >< "none")desc = string('Beim Testen des Systems wurde in der Datei /etc/cups/client.conf, die Einträge -ServerName- und -Encryption- nicht gefunden.');
      }else if(ServerName >!< "none" && Encryption >!< "none"){
        if ("lways" >< Encryption){
          result = string("unvollständig");
          desc = string('Beim Testen des Systems wurden folgende Einträge in der Datei /etc/cups/client.conf, gefunden:\n' + ServerName + '\n' + Encryption + '\nBitte Prüfen Sie, ob die Option -ServerName- den Anforderungen der Maßnahme 5.145 genügt.');
        }
        else{
          result = string("nicht erfüllt");
          desc = string('Beim Testen des Systems wurden folgende Einträge in der Datei /etc/cups/client.conf, gefunden:\n' + ServerName + '\n' + Encryption + '\nDie Option -Encryption- sollte auf -Always- gesetzt sein. Bitte Prüfen Sie, ob die Option -ServerName- den Anforderungen der Maßnahme 5.145 genügt.');
        }
      }
    }
  }
...
#Only Server Installed
    ...
    if (cupsd >< "no cupsd.conf"){
      result = string("nicht erfüllt");
      desc = string('Beim Testen des Systems wurde festgestellt, dass der CUPS-Server installiert ist.\nAllerdings wurde die Datei "/etc/cups/cupsd.conf" nicht gefunden.\nDemnach kann das System nicht entsprechend Massnahme 5.145 konfiguriert sein.');
    }
    else if (cupsd >< "empty"){
      result = string("nicht erfüllt");
      desc = string('Beim Testen des Systems wurde festgestellt, dass der CUPS-Server installiert ist.\nAllerdings ist die Datei "/etc/cups/cupsd.conf" leer.\nDemnach kann das System nicht entsprechend Massnahme 5.145 konfiguriert sein.');  
    }else{
      if(Listen >< "none" || Browsing >< "none" || LogLevel >< "none" || PreserveJobs >< "none" || AdminResult >< "none" || DefaultAuthType >< "none"){
        result = string("nicht erfüllt");
        desc = string('Beim Testen des Systems wurden in der Datei "/etc/cups/cupsd.conf" folgende Einträge nicht gefunden: \n');
        ...
        result = string("unvollständig");
        desc = string('\nBitte Prüfen Sie, ob die Optionen -Listen-, -Browsing-, -LogLevel-, -PreserveJobs- und -<Location /admin>- den Anforderungen der Maßnahme 5.145 genügen: \n' + Listen + '\n' + Browsing + '\n' + LogLevel + '\n' + PreserveJobs + '\n' + DefaultAuthType + '\n' + AdminResult + '\n');
      }  
    }
  }
...
#Both installed
    ...
    if (cupsclient >< "no client.conf"){
      ...
      if(ServerName >< "none" || Encryption >< "none" || Listen >< "none" || Browsing >< "none" || LogLevel >< "none" || PreserveJobs >< "none" || AdminResult >< "none" || DefaultAuthType >< "none"){
        ...
        if (DefaultAuthType >< "none") desc += string('DefaultAuthType, ');
        if (AdminResult >< "none") desc += string('<Location /admin>, ');
        if (ServerName >< "none" || Encryption >< "none")desc += string('\nBeim Testen des Systems wurden in der Datei /etc/cups/client.conf folgende Einträge nicht gefunden: \n');
        ...
        if ("lways" >< Encryption){
          result = string("unvollständig");
          desc = string('Beim Testen des Systems wurden folgende Einträge in der Datei /etc/cups/client.conf, gefunden:\n' + ServerName + '\n' + Encryption + '\n' + 'Beim Testen des Systems wurden folgende Einträge in der Datei /etc/cups/cupsd.conf, gefunden:\n' + Listen + '\n' + Browsing + '\n' + LogLevel + '\n' + PreserveJobs + '\n' + DefaultAuthType + '\n' + AdminResult + 'Bitte Prüfen Sie, ob die Optionen -ServerName-, -Listen-, -Browsing-, -LogLevel-, -PreserveJobs- und -<Location /admin>- den Anforderungen der Maßnahme 5.145 genügen.');
        }else{
          result = string("nicht erfüllt");
...
set_kb_item(name:"GSHB-10/M5_145/result", value:result);
set_kb_item(name:"GSHB-10/M5_145/desc", value:desc);
set_kb_item(name:"GSHB-10/M5_145/name", value:name);
