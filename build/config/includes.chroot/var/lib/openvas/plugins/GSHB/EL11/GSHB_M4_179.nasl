...
#
# IT-Grundschutz, 11. EL, Maﬂnahme 4.179
#
# Authors:
...
if (! OPENVAS_VERSION)
{
        set_kb_item(name:"GSHB-11/M4_179/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-11/M4_179/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-11/M4_179/name", value:name);
...
if (WMIOSLOG == "On the Target System runs Samba, it is not an Microsoft System."){
  ...
  if (log) desc = string("Beim Testen des Systems trat ein Fehler auf:\n" + log);
}else if("None" >< AdmTools){
  result = string("erf¸llt");
  desc = string("AdminTools wurde nicht im Verzeichnis %windir% bzw.\n%windir%\\system32 gefunden.");
  if (WinVer == '5.0') desc = desc + string('\nAchten Sie darauf, dass auf %systemdrive% (C:\\), der\nBenutzer Everyone(Jeder) nur eingeschr‰nkte\nZugriffsrechte erh‰lt!');
}else if("inapplicable" >< AdmTools){
  result = string("nicht zutreffend");
  desc = string("Auf dem System ist kein IIS istalliert.");
}else if(AdmTools){
  result = string("nicht erf¸llt");
  desc = string('\nFolgende Admintools wurden noch gefunden:\n' + AdmTools);
  if (WinVer == '5.0') desc = desc + string('\nAchten Sie weiterhin darauf, dass auf %systemdrive%\n(C:\\), der Benutzer Everyone(Jeder) nur\neingeschr‰nkte Zugriffsrechte erh‰lt!');
}

set_kb_item(name:"GSHB-11/M4_179/result", value:result);
set_kb_item(name:"GSHB-11/M4_179/desc", value:desc);
set_kb_item(name:"GSHB-11/M4_179/name", value:name);
