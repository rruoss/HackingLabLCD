...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 4.052
#
# Authors:
...
if (WMIOSLOG == "On the Target System runs Samba, it is not an Microsoft System."){
  ...
  if(!cdlog) desc = string("Beim Testen des Systems trat ein Fehler auf, siehe Log Message!");
  if(cdlog) desc = string("Beim Testen des Systems trat ein Fehler auf: " + cdlog + '\n' + fdlog);
  ...
  desc = string("FD und CD-Zugriff nur f¸r den lokalen Benutzer freigegeben.");
} else if(cdalloc >< "off" && fdalloc >< "on"){
  result = string("nicht erf¸llt");
  desc = string("Nur FD-Zugriff f¸r den lokalen Benutzer freigegeben. CD Zugriff ist weiterhin ¸ber Netzwerk mˆglich");
} else if(cdalloc >< "on" && fdalloc >< "off"){
  result = string("nicht erf¸llt");
  desc = string("Nur CD-Zugriff f¸r den lokalen Benutzer freigegeben. FD Zugriff ist weiterhin ¸ber Netzwerk mˆglich");
} else {
  result = string("nicht erf¸llt");
  desc = string("FD und CD-Zugriff nicht nur f¸r den lokalen Benutzer freigegeben!");
}
