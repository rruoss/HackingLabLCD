...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 4.287
#
# Authors:
...
if (! OPENVAS_VERSION)
  {
        set_kb_item(name:"GSHB-10/M4_287/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-10/M4_287/desc", value:"Leider setzen Sie einen zu alte OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-10/M4_287/name", value:name);
...
if(!sip){
    ...
    desc = string('Auf dem Zielsystem wurde kein SIP-Dienst gefunden. Davon ausgehend, wird es nicht als VoIP-Middleware behandelt.');
}else if (!http && !val){
    result = string("erf¸llt");
    if (!http && !val) desc = string('Auf dem Zielsystem wurde weder ein HTTP-Server-Port 80 noch die SSH-Protokollversion 1 gefunden.');
    else if (!http) desc = string('Auf dem Zielsystem wurde kein HTTP-Server-Port 80 gefunden.');
    else if (!val) desc = string('Auf dem Zielsystem wurde die SSH-Protokollversion 1 nicht gefunden.');
}else {
    result = string("nicht erf¸llt");
    if (http && val) desc = string('Auf dem Zielsystem wurde ein HTTP-Server-Port 80 und die SSH-Protokollversion 1 gefunden.\nEine Web-basierte Konfiguration sollte immer gesichert erfolgen, beispielsweise durch den Einsatz von SSL oder TLS.\nIhre SSH Einstellungen lassen Verbindungen mit der Protokollversion 1 zu.\nDiese Version enth‰lt Schwachstellen. Sie sollten nur die Protokollversion 2 einsetzten.');
    else if (!http) desc = string('Auf dem Zielsystem wurde ein HTTP-Server-Port 80 gefunden.\nEine Web-basierte Konfiguration sollte immer gesichert erfolgen, beispielsweise durch den Einsatz von SSL oder TLS.');
    else if (!val) desc = string('Auf dem Zielsystem wurde die SSH-Protokollversion 1 gefunden.\nIhre SSH Einstellungen lassen Verbindungen mit der Protokollversion 1 zu.\nDiese Version enth‰lt Schwachstellen. Sie sollten nur die Protokollversion 2 einsetzten.');
}
...
set_kb_item(name:"GSHB-10/M4_287/result", value:result);
set_kb_item(name:"GSHB-10/M4_287/desc", value:desc);
set_kb_item(name:"GSHB-10/M4_287/name", value:name);
