...
#
# IT-Grundschutz, 11. EL, Maﬂnahme 4.277
#
# Authors:
...
if (! OPENVAS_VERSION)
{
        set_kb_item(name:"GSHB-11/M4_277/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-11/M4_277/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-11/M4_277/name", value:name);
...
}else if (NTLMMinServerSec == "537395248" && requiresignorseal == "1" && requirestrongkey == "1" && RequireSecuritySignatureWs == "1" && EnablePlainTextPassword == "0" && RequireSecuritySignatureSvr == "1" && EnableSecuritySignatureSvr == "1" && NoLMHash == "1" && lmcomplevel >= "5" && LDAPClientIntegrity == "1" && NTLMMinClientSec == "537395248")
{
  ...
  if (NTLMMinClientSec != "537395248") val = val + '\n\n' + "Netzwerksicherheit: Minimale Sitzungssicherheit f¸r\nNTLM-SSP-basierte Clients (einschlieﬂlich sicherer\nRPC-Clients)";

  desc = string("Die Sicherheitseinstellung stimmen nicht mit der\nMaﬂnahme M4.277 ‹berein. Folgende Einstellungen sind\nnicht wie im Dokument 'Windows Server 2003 Security\nBaseline Settings' gefordert umgesetzt: " + val);
}


set_kb_item(name:"GSHB-11/M4_277/result", value:result);
set_kb_item(name:"GSHB-11/M4_277/desc", value:desc);
set_kb_item(name:"GSHB-11/M4_277/name", value:name);
