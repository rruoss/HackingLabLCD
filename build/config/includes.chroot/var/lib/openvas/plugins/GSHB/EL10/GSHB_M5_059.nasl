...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 5.059
#
# Authors:
...
if (! OPENVAS_VERSION)
  {
        set_kb_item(name:"GSHB-10/M5_059/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-10/M5_059/desc", value:"Leider setzen Sie einen zu alte OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-10/M5_059/name", value:name);
...
if(OSNAME >!< "none"){
  ...
  desc = string('Auf dem System wurde keine hosts Konfiguration in /etc/nsswitch.conf gefunden.'); 
}else if(nsswitch >!< "none" && nsswitch >!< "nogrep" && nsswitch !~ 'hosts:[ \t]+files.*'){
  result = string("nicht erf¸llt");
  desc = string('Auf dem System wurde in der Hosts Konfiguration in /etc/nsswitch.conf, kein -files- \nEintrag gefunden bzw., er steht nicht wie gefordert an erster Stelle:\n\n' + nsswitch); 
}else if(nsswitch >!< "none" && nsswitch >!< "nogrep"){
  result = string("erf¸llt");
  desc = string('Bitte Pr¸fen Sie, ob die Ergebnisse den Anforderungen der Maﬂnahme 5.059 entsprechen!\nAuf dem System konnten folgende Hosts Konfiguration in /etc/nsswitch.conf gefunden:\n\n' + nsswitch + '\nFolgende Einstellungen wurden in /etc/hosts gefunden:\n\n' + hosts); 
}else if (nsswitch >< "nogrep" || hosts >< "nogrep"){
  result = string("Fehler");
  desc += string('Beim Testen des Systems, wurde der Befehl grep nicht gefunden.'); 
}

set_kb_item(name:"GSHB-10/M5_059/result", value:result);
set_kb_item(name:"GSHB-10/M5_059/desc", value:desc);
set_kb_item(name:"GSHB-10/M5_059/name", value:name);
