...
#
# IT-Grundschutz, 11. EL, Maﬂnahme 4.186
#
# Authors:
...
if (! OPENVAS_VERSION)
{
        set_kb_item(name:"GSHB-11/M4_186/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-11/M4_186/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-11/M4_186/name", value:name);
...
if (WMIOSLOG == "On the Target System runs Samba, it is not an Microsoft System."){
  ...
  desc = string("Beispieldateien und Administrations-Skripte des IIS\nsind nicht installiert.");
} else {
  result = string("nicht erf¸llt");
  desc = string("Die Anforderungen der Maﬂnahme M4.186 wurden\nnicht erf¸llt:\n");
  if (!isnull(iissam)) desc += string("Das Verzeichnis 'c:\\inetpub\\iissamples' wurde\nnicht entfernt.\n");
  if (!isnull(iissdk)) desc += string("Das Verzeichnis 'c:\\inetpub\\iissamples\sdk' wurde\nnicht entfernt.\n");
  if (!isnull(iisscr)) desc += string("Das Verzeichnis 'c:\\inetpub\\AdminScripts' wurde\nnicht entfernt.\n");
  if (!isnull(iispwd)) desc += string("Das Verzeichnis\n'%systemroot%\\system32\\inetsrv\\iisadmpwd' wurde\nnicht entfernt.\n");
  if (!isnull(iismadc)) desc += string("Das Verzeichnis\n'c:\\Program Files\\Common Files\\System\\msadc\\Samples'\nbzw. 'c:\\Programme\\Common Files\\System\\msadc\\Samples'\nwurde nicht entfernt.\n");
}

set_kb_item(name:"GSHB-11/M4_186/result", value:result);
set_kb_item(name:"GSHB-11/M4_186/desc", value:desc);
set_kb_item(name:"GSHB-11/M4_186/name", value:name);
