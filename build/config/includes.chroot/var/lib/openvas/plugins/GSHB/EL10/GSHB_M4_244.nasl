...
# $Id: GSHB_M4_244.nasl 9 2013-10-27 09:38:41Z jan $
#
...
tag_summary = "IT-Grundschutz M4.244: Sichere Windows XP Systemkonfiguration(Win).
...
}else
{
  ...
  if (AddPrinterDrivers != "1") val = val + '\n' + "Geräte: Anwendern das Installieren von Druckertreibern nicht erlauben: " + AddPrinterDrivers;
  if (AllocateCDRoms != "1") val = val + '\n' + "Geräte: Zugriff auf CD-ROM-Laufwerke auf lokal angemeldete Benutzer beschränken: " + AllocateCDRoms;
  if (AllocateDASD != "2") val = val + '\n' + "Geräte: Formatieren und Auswerfen von Wechselmedien zulassen: " + AllocateDASD;
  if (AllocateFloppies != "1") val = val + '\n' + "Geräte: Zugriff auf Diskettenlaufwerke auf lokal angemeldete Benutzer beschränken: " + AllocateFloppies;
  ...
  if (AuditBaseObjects != "0") val = val + '\n' + "Überwachung: Zugriff auf globale Systemobjekte prüfen: " + AuditBaseObjects;
  ...
  if (autodisconnect != "15") val = val + '\n' + "Microsoft-Netzwerk (Server): Leerlaufzeitspanne bis zum Anhalten der Sitzung: " + autodisconnect;
  if (cachedlogonscount != "0") val = val + '\n' + "Interaktive Anmeldung: Anzahl zwischenzuspeichernder vorheriger Anmeldungen (für den Fall, dass der Domänencontroller nicht verfügbar ist): " + cachedlogonscount;
  if (ClearPageFileAtShutdown != "1") val = val + '\n' + "Herunterfahren: Auslagerungsdatei des virtuellen Arbeitspeichers löschen: " + ClearPageFileAtShutdown;
  if (ClearTextPassword != "False") val = val + '\n' + "Kennwörtern für alle Domänenbenutzer mit umkehrbarer Verschlüsselung speichern: " + ClearTextPassword;
  if (crashonauditfail != "0") val = val + '\n' + "Überwachung: System sofort herunterfahren, wenn Sicherheitsüberprüfungen nicht protokolliert werden können: " + crashonauditfail;
  if (DisableCAD != "0") val = val + '\n' + "Interaktive Anmeldung: Kein STRG+ALT+ENTF erforderlich: " + DisableCAD;
  if (DisableDomainCreds != "1") val = val + '\n' + "Netzwerkzugriff: Speicherung von Anmeldeinformationen oder .NET-Passports für die Netzwerkauthentifikation nicht erlauben: " + DisableDomainCreds;
  ...
  if (DontDisplayLastUserName != "on") val = val + '\n' + "Interaktive Anmeldung: Letzten Benutzernamen nicht anzeigen: " + DontDisplayLastUserName;
  if (enableforcedlogoff != "1") val = val + '\n' + "Microsoft-Netzwerk (Server): Clientverbindungen aufheben, wenn die Anmeldezeit überschritten wird: " + enableforcedlogoff;
  if (EnablePlainTextPassword != "0") val = val + '\n' + "Microsoft-Netzwerk (Client): Unverschlüsseltes Kennwort an SMB-Server von Drittanbietern senden: " + EnablePlainTextPassword;
  if (EnableSecuritySignatureSvr != "1") val = val + '\n' + "Microsoft-Netzwerk (Client): Kommunikation digital signieren (wenn Server zustimmt): " + EnableSecuritySignatureSvr;
  if (EnableSecuritySignatureWs != "1") val = val + '\n' + "Microsoft-Netzwerk (Server): Kommunikation digital signieren (wenn Client zustimmt): " + EnableSecuritySignatureWs;
  if (EveryoneIncludesAnonymous != "0") val = val + '\n' + "Netzwerkzugriff: Die Verwendung von 'Jeder'-Berechtigungen für anonyme Benutzer ermöglichen: " + EveryoneIncludesAnonymous;
  if (FIPSAlgorithmPolicy != "0") val = val + '\n' + "Systemkryptografie: FIPS-konformen Algorithmus für Verschlüsselung, Hashing und Signatur verwenden: " + FIPSAlgorithmPolicy;
  if (ForceGuest != "0") val = val + '\n' + "Netzwerkzugriff: Modell für gemeinsame Nutzung und Sicherheitsmodellfür lokale Konten: " + ForceGuest;
  if (ForceUnlockLogon != "1") val = val + '\n' + "Interaktive Anmeldung: Domänencontrollerauthentifizierung zum Aufheben der Sperrung der Arbeitsstation erforderlich: " + ForceUnlockLogon;
  if (fullprivilegeauditing != "0") val = val + '\n' + "Überwachung: Die Verwendung des Sicherungs- und Wiederherstellungsrechts überprüfen: " + fullprivilegeauditing;
  if (LDAPClientIntegrity != "1") val = val + '\n' + "Netzwerksicherheit: Signaturanforderungen für LDAP-Clients: " + LDAPClientIntegrity;
  if (ldapserverintegrity != "2") val = val + '\n' + "Domänencontroller: Signaturanforderungen für LDAP-Server: " + ldapserverintegrity;
  if (LegalNoticeCaption != "on") val = val + '\n' + "Interaktive Anmeldung: Nachrichtentitel für Benutzer, die sich anmelden wollen: " + LegalNoticeCaption;
  if (LegalNoticeText != "on") val = val + '\n' + "Interaktive Anmeldung: Nachricht für Benutzer, die sich anmelden wollen: " + LegalNoticeText;
  if (LimitBlankPasswordUse != "1") val = val + '\n' + "Konten: Lokale Kontenverwendung von leeren Kennwörtern auf Konsolenanmeldung beschränken: " + LimitBlankPasswordUse;
  ...
  if (LSAAnonymousNameLookup >< "None") val = val + '\n' + "Netzwerkzugriff: Anonyme SID-/Namensübersetzung zulassen: " + LSAAnonymousNameLookup;
  else{
    if (LSAAnonymousNameLookup[2] != "False") val = val + '\n' + "Netzwerkzugriff: Anonyme SID-/Namensübersetzung zulassen: " + LSAAnonymousNameLookup[2];
  }
  if (MachinePaths != "Empty") val = val + '\n' + "Netzwerkzugriff: Registrierungspfade, auf die von anderen Computern aus zugegriffen werden kann: " + MachinePaths;
  if (maximumComppasswordage > 30) val = val + '\n' + "Domänenmitglied: Maximalalter von Computerkontenkennwörtern: " + maximumComppasswordage;
  ...
  if (nodefaultadminowner != "1") val = val + '\n' + "Systemobjekte: Standardbesitzer für Objekte, die von Mitgliedern der Administratorengruppe erstellt werden: " + nodefaultadminowner;
  if (NoLMHash != "1") val = val + '\n' + "Netzwerksicherheit: Keine LAN Manager-Hashwerte für nächste Kennwortänderung speichern: " + NoLMHash;
  if (NTLMMinClientSec != "537395248") val = val + '\n' + "Netzwerksicherheit: Minimale Sitzungssicherheit für NTLM-SSP-basierte Clients (einschließlich sicherer RPC-Clients: " + NTLMMinClientSec;
  if (NTLMMinServerSec != "537395248") val = val + '\n' + "Netzwerksicherheit: Minimale Sitzungssicherheit für NTLM-SSP-basierte Server (einschließlich sicherer RPC-Server): " + NTLMMinServerSec;
  if (NullSessionPipes != "Empty") val = val + '\n' + "Netzwerkzugriff: Named Pipes, auf die anonym zugegriffen werden kann: " + NullSessionPipes;
  if (NullSessionShares != "Empty") val = val + '\n' + "Netzwerkzugriff: Freigaben, auf die anonym zugegriffen werden kann: " + NullSessionShares;
  if (ObCaseInsensitive != "1") val = val + '\n' + "Systemobjekte: Groß-/Kleinschreibung für Nicht-Windows-Subsysteme ignorieren: " + ObCaseInsensitive;
  ...
  if (PasswordComplexity != "True") val = val + '\n' + "Kennwort muss Komplexitätsvoraussetzungen entsprechen: " + PasswordComplexity;
  if (passwordexpirywarning != "14") val = val + '\n' + "Interaktive Anmeldung: Anwender vor Ablauf des Kennworts zum Ändern des Kennworts auffordern: " + passwordexpirywarning;
  if (PasswordHistorySize < 6) val = val + '\n' + "Kennwortchronik erzwingen: " + PasswordHistorySize;
  if (Policy != "1") val = val + '\n' + "Geräte: Verhalten bei der Installation von nichtsignierten Treibern: " + Policy;
  if (ProtectionMode != "1") val = val + '\n' + "Systemobjekte: Standardberechtigungen interner Systemobjekte (z. B. symbolischer Verknüpfungen) verstärken: " + ProtectionMode;
  if (RequireSecuritySignatureSvr != "1") val = val + '\n' + "Microsoft-Netzwerk (Client): Kommunikation digital signieren (immer): " + RequireSecuritySignatureSvr;
  if (RequireSecuritySignatureWs != "1") val = val + '\n' + "Microsoft-Netzwerk (Server): Kommunikation digital signieren (immer): " + RequireSecuritySignatureWs;
  if (requiresignorseal != "1") val = val + '\n' + "Domänenmitglied: Daten des sicheren Kanals digital verschlüsseln oder signieren (immer): " + requiresignorseal;
  if (requirestrongkey != "1") val = val + '\n' + "Domänenmitglied: Starker Sitzungsschlüssel erforderlich (Windows 2000 oder höher: " + requirestrongkey;
  if (ResetLockoutCount < 30) val = val + '\n' + "Zurücksetzungsdauer des Kontosperrungszählers: " + ResetLockoutCount;
  if (RestrictAnonymous != "1") val = val + '\n' + "Netzwerkzugriff: Anonyme Aufzählung von SAM-Konten und Freigaben nicht erlauben: " + RestrictAnonymous;
  if (RestrictAnonymousSAM != "1") val = val + '\n' + "Netzwerkzugriff: Anonyme Aufzählung von SAM-Konten nicht erlauben: " + RestrictAnonymousSAM;
  if (RestrictGuestAccessApp >< "None") val = val + '\n' + "Lokalen Gastkontozugriff auf Anwendungsprotokoll verhindern: " + RestrictGuestAccessApp;
  else{
    if (RestrictGuestAccessApp[2] != "True") val = val + '\n' + "Lokalen Gastkontozugriff auf Anwendungsprotokoll verhindern: " + RestrictGuestAccessApp[2];
  }
  if (RestrictGuestAccessEvent >< "None") val = val + '\n' + "Lokalen Gastkontozugriff auf Systemprotokoll verhindern: " + RestrictGuestAccessEvent;
  else{
    if (RestrictGuestAccessEvent[2] != "True") val = val + '\n' + "Lokalen Gastkontozugriff auf Systemprotokoll verhindern: " + RestrictGuestAccessEvent[2];
  }
  if (RestrictGuestAccessSec >< "None") val = val + '\n' + "Lokalen Gastkontozugriff auf Sicherheitsprotokoll verhindern: " + RestrictGuestAccessSec;
  else{
    if (RestrictGuestAccessSec[2] != "True") val = val + '\n' + "Lokalen Gastkontozugriff auf Sicherheitsprotokoll verhindern: " + RestrictGuestAccessSec[2];
  }
  if (scremoveoption != "1") val = val + '\n' + "Interaktive Anmeldung: Verhalten beim Entfernen von Smartcards: " + scremoveoption;
  if (sealsecurechannel != "1") val = val + '\n' + "Domänenmitglied: Daten des sicheren Kanals digital verschlüsseln (wenn möglich): " + sealsecurechannel;
  if (securitylevel != "0") val = val + '\n' + "Wiederherstellungskonsole: Automatische administrative Anmeldungen zulassen: " + securitylevel;
  if (setcommand != "0") val = val + '\n' + "Wiederherstellungskonsole: Kopieren von Disketten und Zugriff auf alle Laufwerke und alle Ordner zulassen: " + setcommand;
  if (ShutdownWithoutLogon != "0") val = val + '\n' + "Herunterfahren: Herunterfahren des Systems ohne Anmeldung zulassen: " + ShutdownWithoutLogon;
  if (signsecurechannel != "1") val = val + '\n' + "Domänenmitglied: Daten des sicheren Kanals digital signieren (wenn möglich): " + signsecurechannel;
  if (undockwithoutlogon != "0") val = val + '\n' + "Geräte: Entfernen ohne vorherige Anmeldung erlauben: " + undockwithoutlogon;

  if(NoUpdateCheck == "0") val = val + '\n' + "Disable Periodic Check for Internet Explorer Software updates: " + NoUpdateCheck;
  else if (NoUpdateCheck >< "None")  val = val + '\n' + "Disable Periodic Check for Internet Explorer Software updates wurde nicht konfiguriert";
  if(PreventAutoRun == "0") val = val + '\n' + "Windows Messenger\Windows Messenger nicht automatisch starten: " + PreventAutoRun;
  else if (PreventAutoRun >< "None")  val = val + '\n' + "Windows Messenger\Windows Messenger nicht automatisch starten wurde nicht konfiguriert";
  ...
  if(Headlines == "1") val = val + '\n' + "'Wussten Sie schon' -Inhalte im Hilfe- und Supportcenter deaktivieren: " + Headlines;
  else if (Headlines >< "None")  val = val + '\n' + "'Wussten Sie schon' -Inhalte im Hilfe- und Supportcenter deaktivieren wurde nicht konfiguriert";
  if(DisableWindowsUpdateAccess == "0") val = val + '\n' + "Zugriff auf alle Windows Update-Funktionen deaktivieren: " + DisableWindowsUpdateAccess;
  else if (DisableWindowsUpdateAccess >< "None")  val = val + '\n' + "Zugriff auf alle Windows Update-Funktionen deaktivieren wurde nicht konfiguriert";
  if(DontSearchWindowsUpdate == "0") val = val + '\n' + "Suche nach Gerätetreibern auf Windows Update deaktivieren: " + DontSearchWindowsUpdate;
  else if (DontSearchWindowsUpdate >< "None")  val = val + '\n' + "Suche nach Gerätetreibern auf Windows Update deaktivieren wurde nicht konfiguriert";
  if(NoRegistration == "0") val = val + '\n' + "Registrierung deaktivieren, wenn sich dir URL-Verbindung auf microsoft.com bezieht: " + NoRegistration;
  else if (NoRegistration >< "None")  val = val + '\n' + "Registrierung deaktivieren, wenn sich dir URL-Verbindung auf microsoft.com bezieht wurde nicht konfiguriert";
  if(DisableRootAutoUpdate == "0") val = val + '\n' + "Automatischer Update von Stammzertifikaten deaktivieren: " + DisableRootAutoUpdate;
  else if (DisableRootAutoUpdate >< "None")  val = val + '\n' + "Automatischer Update von Stammzertifikaten deaktivieren wurde nicht konfiguriert";
  if(MicrosoftEventVwrDisableLinks == "0") val = val + '\n' + "Event.asp-Links der ereignisanzeige deaktivieren: " + MicrosoftEventVwrDisableLinks;
  else if (MicrosoftEventVwrDisableLinks >< "None")  val = val + '\n' + "Event.asp-Links der ereignisanzeige deaktivieren wurde nicht konfiguriert";
  if(NoInternetOpenWith == "0") val = val + '\n' + "Internet-Dateizuordnungsdienst deaktivieren: " + NoInternetOpenWith;
  else if (NoInternetOpenWith >< "None")  val = val + '\n' + "Internet-Dateizuordnungsdienst deaktivieren wurde nicht konfiguriert";
  if(DoReport == "1") val = val + '\n' + "Fehlerberichterstattung deaktivieren: " + DoReport;
  else if (DoReport >< "None")  val = val + '\n' + "Fehlerberichterstattung deaktivieren wurde nicht konfiguriert";
