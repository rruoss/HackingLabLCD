...
if (! OPENVAS_VERSION)
{
        set_kb_item(name:"GSHB-10/M5_034/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-10/M5_034/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers.");
        set_kb_item(name:"GSHB-10/M5_034/name", value:name);
...
if(OSNAME >!< "none"){
  ...
  result = string("Fehler");
  if (OPIPAM == "norights") desc = string('Der Testuser hat kein Recht auf die Datei /etc/pam.d/opie zu lesen.');
#  if (OPISSH == "norights") desc += string('\nDer Testuser hat kein Recht auf die Datei /etc/ssh/sshd_config zu lesen.');
  if (OPISERVICES == "norights") desc += string('\nDer Testuser hat kein Recht die Dateien unter /etc/pam.d/ zu lesen');
  ...
  if ( "auth sufficient pam_opie.so" >< OPIPAM && "auth required pam_deny.so" >< OPIPAM ){
    ...
    if (OPISSH == "norights") desc = string('\nDer Testuser hat kein Recht auf die Datei /etc/ssh/sshd_config zu lesen. In dieser Datei sollte der Eintrag -ChallengeResponseAuthentication yes- stehen, damit auch SSH mit Einmalpasswörtern arbeiten kann.');
...
set_kb_item(name:"GSHB-10/M5_034/result", value:result);
set_kb_item(name:"GSHB-10/M5_034/desc", value:desc);
set_kb_item(name:"GSHB-10/M5_034/name", value:name);
