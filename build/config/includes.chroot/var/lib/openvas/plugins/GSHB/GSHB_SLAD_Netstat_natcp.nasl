...
function run_slad_netstat (sock, slad_exe) {
  global_var netstatresult;
  slad_cmd = slad_exe + " -r Netstat:natcp";
  results = ssh_cmd (socket: sock, cmd: slad_cmd, timeout: 30);
  if (results)  netstatresult = results; 
}
...
if(!sock) {
  ...
  set_kb_item(name:"GSHB/SLAD/NETSTAT", value:"nosock");  
  set_kb_item(name:"GSHB/SLAD/NETSTAT/log", value:error);    
  exit(0);
}else run_slad_netstat (sock: sock, slad_exe: "/opt/slad/bin/sladd");
  


if ("403 plugin already running" >< netstatresult || "200 queued" >< netstatresult)
{
  ...
  while (bhead) {
    ...
    if (parts[0] == "R" && parts[1] == "Netstat" && parts[2] == "natcp" && parts[3] == "netstat-natcp") {
      running += string (desc + "\n");
    } else if (parts[0] == "T" && parts[1] == "Netstat" && parts[2] == "natcp" && parts[3] == "netstat-natcp") {
      results += string (desc + "\n");
  ...
  else if (!running) set_kb_item(name:"GSHB/SLAD/NETSTAT", value:results);
  else if (running) set_kb_item(name:"GSHB/SLAD/NETSTAT", value:"running");
}else {
  if (!netstatresult) netstatresult = "none";
  if ( "/opt/slad/bin/sladd: No such file or directory" >< netstatresult || "/opt/slad/bin/sladd: Datei oder Verzeichnis nicht gefunden" >< netstatresult  || "sh: /opt/slad/bin/sladd: not found"  >< netstatresult ) netstatresult = "noslad";
  set_kb_item(name:"GSHB/SLAD/NETSTAT", value:netstatresult);
}
