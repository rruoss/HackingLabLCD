...
if (! OPENVAS_VERSION)
  {
        set_kb_item(name:"GSHB-10/M5_009/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-10/M5_009/desc", value:"Leider setzen Sie einen zu alte OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers.");
        set_kb_item(name:"GSHB-10/M5_009/name", value:name);
...
if (OSNAME >!< "none"){
  if(!OSNAME){
    ...
    if (WINLOG) desc = string("Beim Testen des Systems trat ein Fehler auf: " + log);
    ...
    desc = string("Das System ist ein Windows Serverbetriebssystem und kann zur Zeit noch nicht getestet werden.");  
  }
}else{
  if(logwatch >< "nosock"){
    result = string("Fehler");
   ...
   if (log) desc = string("Beim Testen des Systems trat beim Verbinden ¸ber SSH ein Fehler auf: " + log);
    ...
    result = string("Fehler");
    desc = string('Beim Testen des Systems wurde festgestellt, dass das notwendige logwatch Plugin nicht installiert ist.');
  ...
  result = string("unvollst‰ndig");
  if (verbose == "yes")desc = string('Bitte pr¸fen Sie die Eintr‰ge im nachfolgenden Logfile:\n' + logwatch);
#  if (verbose == "yes" || (verbose != "yes"  && loglenght <= 2000))desc = string('Bitte pr¸fen Sie die Eintr‰ge im nachfolgenden Logfile:\n' + logwatch);  
  else desc = string('Das Logfile ist ' + loglenght + ' Zeichen lang. F¸r eine vollst‰ndige Liste, w‰hlen Sie bei den Voreinstellungen dieses Tests: Alle Logfile-Eintr‰ge Auflisten');
  }
}
if (!result){
  result = string("Fehler");
...
set_kb_item(name:"GSHB-10/M5_009/result", value:result);
set_kb_item(name:"GSHB-10/M5_009/desc", value:desc);
set_kb_item(name:"GSHB-10/M5_009/name", value:name);
