...
if(OSNAME >!< "none"){
  ...
  desc = string('Das System ist ein ' + OSNAME + ' System.\nZur Zeit gibt es die Slad-Plugins Tiger und Tripwire nicht für Windows Systeme.');
}else if (ssh != "ok"){
  result = string("Fehler");
  desc = string('Um das System mit SLAD zu Testen, benötigen Sie eine SSH Verbindung.\nFolgender Fehler ist aufgetreten:' + ssh);
}else if (sladlst == "noslad"){
  result = string("Fehler");
  desc = string('Auf dem System ist SLAD nicht installiert. Demnach kann das System nicht gemäß Maßnahme 4.093 getestet werden.');
  ...
  desc += string('\nDemnach kann das System nicht gemäß Maßnahme 4.093 getestet werden.');
}else{
  result = string("unvollständig");
  desc = string('Auf dem System ist Slad mit den Slad Plugins Tiger und Tripwire installiert.\nFühren Sie bitte eine OpenVAS-Prüfung Ihres Netzwerkes mit den genannten SLAD Plugins aus.');
}
