...
# $Id: secpod_ms08-067_900056.nasl 16 2013-10-27 13:09:52Z jan $
...
if(description)
{
  ...
  script_summary("Check for Remote Code Execution (Exploit)");
  desc = "
  Summary:
  " + tag_summary + "
...
## Check the response
smb_nt_andx_resp = smb_recv(socket:soc, length:4096);
...
## Check the response
dcerpc_bind_srvsvc_resp = smb_recv(socket:soc, length:4096);
...
## Check the response
smb_andx_resp = smb_recv(socket:soc, length:4096);
...
##Check the response
smb_nt_andx_resp1 = smb_recv(socket:soc, length:4096);
...
## Check the response
dcerpc_bind_wkssvc_resp = smb_recv(socket:soc, length:4096);
...
## Check the response
smb_andx_resp1 = smb_recv(socket:soc, length:4096);
...
## Check the response
netpath_cmp_resp = smb_recv(socket:soc, length:4096);
...
## Check the response
smb_tree_dis_resp = smb_recv(socket:soc, length:4096);
...
## Check the response
smb_logoff_resp = smb_recv(socket:soc, length:4096);
