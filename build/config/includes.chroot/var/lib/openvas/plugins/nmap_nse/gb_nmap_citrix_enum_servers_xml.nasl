...
# $Id: gb_nmap_citrix_enum_servers_xml.nasl 10 2013-10-27 10:03:59Z jan $
#
...
tag_summary = "This script attempts to extract the name of the server farm and
  member servers from Citrix XML service.

  This is a wrapper on the Nmap Security Scanner's (http://nmap.org) citrix-enum-servers-xml.nse.";
...
if(description)
{
  ...
  script_summary("Extracts the name of the server farm and member servers from Citrix XML service");
...
argv = make_list("nmap", "--script=citrix-enum-servers-xml.nse", "-p", port,
                 get_host_ip());
