...
# $Id: dcetest.nasl 41 2013-11-04 19:00:12Z jan $
# Description: DCE Services Enumeration
#
# Authors:
...
tag_summary = "Distributed Computing Environment (DCE) services running on the remote host 
...
full_report = myprereport;
  ...
  if(!isnull(all_report_udp))
  {
  	foreach port (keys(all_report_udp)) {
	   x++;
           full_report += string("Port: ", port,"/udp\n\n",all_report_udp[port]);
	}	
 }

  if(x > 0) {

    full_report += mypostreport;
    security_warning(port:135, data:full_report);

  }  

  if(all_report_135 != NULL)
	security_warning(port:135, data:myprereport + all_report_135 + mypostreport);
