...
# $Id: gb_vtiger_crm_detect.nasl 44 2013-11-04 19:58:48Z jan $
#
...
tag_summary = "Detection of Symantec vtiger CRM.
...
foreach dir (dirs) {
 ...
 if("<title>vtiger CRM" >< buf && "login_language" >< buf)
 {
    ...
    set_kb_item(name:"vtiger/installed",value:TRUE);

    cpe = build_cpe(value:vers, exp:"^([0-9.]+)", base:"cpe:/a:vtiger:vtiger_crm:");
    if(!cpe)
      cpe = 'cpe:/a:vtiger:vtiger_crm';

    register_product(cpe:cpe, location:install, nvt:SCRIPT_OID, port:port);

    log_message(data: build_detection_report(app:"vtiger CRM", version:vers, install:install, cpe:cpe, concluded: version[0]),
                port:port);
