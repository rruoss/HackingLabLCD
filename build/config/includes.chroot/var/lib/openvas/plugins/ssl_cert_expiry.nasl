...
# $Id: ssl_cert_expiry.nasl 17 2013-10-27 14:01:43Z jan $
...
tag_summary = "The remote server's SSL certificate has already expired or will expire
shortly.
...
tag_solution = "Purchase or generate a new SSL certificate to replace the existing
one.";
...
if (description) {
 ...
 script_version("$Revision: 17 $");
  ...
  script_name("SSL Certificate Expiry");
  script_summary("Checks SSL certificate expiry");

  # (deprecated: once openvas-libaries < 6.0 are not supported anymore
  #  this script can be removed in favor of gb_ssl_cert_expiray.nasl)
...
if (defined_func("cert_open")) {
  ...
  if (!certobj) {
    security_warning(data:string("The SSL certificate of the remote service ",
                                 "can't be parsed!"),
                     port:port);
    exit(0);
  }
  ...
  if (valid_start > now) {
    security_note(data:string("The SSL certificate of the remote service ",
                              "is not valid before ",
    ...
    if (valid_start =~ "^[0-9]{12}$" && valid_end =~ "^[0-9]{12}$") {
      ...
      if (valid_start > now) {
        ...
        security_note(
         ...
         log_message(port:port,data:string("The SSL certificate of the remote service is valid between ", valid_start_alt, " and ", valid_end_alt, "."));
      }
    }
  }
}
