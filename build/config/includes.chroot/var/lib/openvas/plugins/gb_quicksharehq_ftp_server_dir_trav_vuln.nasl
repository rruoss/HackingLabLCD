...
# $Id: gb_quicksharehq_ftp_server_dir_trav_vuln.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_insight = "The flaw is due to an error while handling certain requests containing
  'dot dot' sequences (..) and back slashes in URL, which can be exploited to
  download arbitrary files from the host system via directory traversal attack.";
...
tag_summary = "The host is running QuickShare File Share FTP Server and is prone to
...
if(description)
{
  ...
  script_summary("Try Directory Traversal Attack on QuickShare File Share FTP server");
...
if(login_details)
{
  ...
  if(ftpPort2)
  {
    ## Open a Socket for recieving the data
