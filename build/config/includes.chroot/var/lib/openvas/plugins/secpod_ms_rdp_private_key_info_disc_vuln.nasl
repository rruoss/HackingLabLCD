...
# $Id: secpod_ms_rdp_private_key_info_disc_vuln.nasl 12 2013-10-27 11:15:33Z jan $
#
...
tag_solution = "No solution or patch is available as of July 1, 2012. Information
...
tag_affected = "Microsoft RDP 5.2 and below";
tag_insight = "The flaw is due to RDP server which stores an RSA private key used
...
if(description)
{
  ...
  script_summary("Determine if RDP Server Private Key is available");
...
## Construct RDP Connection Req
req = raw_string(0x03, 0x00, 0x00, 0x0b, 0x06, 0xe0, 0x00, 0x00,
                 0x00, 0x00, 0x00);

## Send RDP Connection Req
send(socket:soc, data:req);
resp = recv(socket:soc, length:5);
if(!resp)
{
  close(soc);
  exit(0);
...
##Confirm if it is RDP Service is running
if(resp =~ "^0300000b06$" )
{
