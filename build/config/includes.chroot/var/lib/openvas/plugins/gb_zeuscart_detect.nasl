...
# $Id: gb_zeuscart_detect.nasl 14 2013-10-27 12:33:37Z jan $
#
...
tag_summary = "The script detects the version of ZeusCart on remote host
  and sets the KB.";
...
foreach dir(make_list("/Zeuscart", "/zeuscart", "/", cgi_dirs()))
{
  ...
  zcReq = http_get (item: string (dir,"/index.php"), port:zcPort);
  zcRes = http_keepalive_send_recv(port:zcPort,data:zcReq);

  ## Confirm the application and Get version
  zcVer = eregmatch(pattern:'<title> ZeusCart V(.[0-9.]+)', string:zcRes);
  if(zcVer[1])
  {
