...
# $Id: secpod_ms09-044.nasl 15 2013-10-27 12:49:54Z jan $
#
...
tag_insight = "- Error exists when processing specific parameters returned by the RDP server,
...
if(sysPath)
{
  dllVer = get_file_version(sysPath, file_name:"Mstscax.dll");
  if(!dllVer){
     exit(0);
  }
}
...
if(sysPath)
{
  dllVer = get_file_version(sysPath, file_name:"System32\Mstscax.dll");
  if(!dllVer){
    exit(0);
  }
}
...
if(hotfix_check_sp(winVista:3) > 0)
{
  ...
  {
    # Grep for Mstscax.dll version < 6.0.6001.18266
  ...
  if("Service Pack 2" >< SP)
  {
    # Grep for Mstscax.dll version < 6.0.6002.18045
