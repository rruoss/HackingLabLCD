...
# $Id: gb_hp_inode_mgmt_center_inodemngchecker_bof_vuln.nasl 12 2013-10-27 11:15:33Z jan $
#
...
tag_affected = "HP iNode Management Center iNode PC 5.1 E0303 and prior";
tag_insight = "The flaws are present due to error in the iNOdeMngChecker.exe component which
...
tag_summary = "This host is installed with HP iNode Management Center and is prone
...
## Check the installation and vulnerable file existance
...
if(imcPath && "Could not find the install Location" >!< imcPath)
{
  ...
  {
    ## Get the version
    imcVer = get_app_version(cpe:CPE, nvt:SCRIPT_OID);

    ## Check for HP iMC 5.0 < 5.1 Patch Kit E0304
    if(imcVer &&
