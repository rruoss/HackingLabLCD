...
# $Id: gb_n_13_news_detect.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_summary = "This script finds the installed N-13 News version and saves
  the result in KB.";
...
foreach path (make_list("/", "/news", cgi_dirs()))
{
  ## Send and receive response
  sndReq = http_get(item:string(path, "/admin.php"), port:newsPort);
  rcvRes = http_send_recv(port:newsPort, data:sndReq);

  ## Confirm the application
  if("<title>N-13 News" >< rcvRes)
  {
    ## Grep for the version
    newsVer = eregmatch(pattern:">N-13 News ([0-9.]+)", string:rcvRes);
    if(newsVer[1] != NULL)
    {
      ## Set the KB value
