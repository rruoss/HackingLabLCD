...
# $Id: secpod_php_nuke_detect.nasl 43 2013-11-04 19:51:40Z jan $
#
...
tag_summary = "This script detects the installed PHP-Nuke version and sets
  the result in KB.";
...
foreach dir (make_list("/php-nuke", "/phpnuke", "/", cgi_dirs()))
{
  sndReq = http_get(item:string(dir, "/html/index.php"), port:nukePort);
  rcvRes = http_send_recv(port:nukePort, data:sndReq);
  if(rcvRes == NULL)
  {
    sndReq = http_get(item:string(dir, "/html/admin.php"), port:nukePort);
