...
# $Id: gb_pidgin_libpurple_mult_dos_vuln_win.nasl 14 2013-10-27 12:33:37Z jan $
#
...
tag_impact = "Attackers can exploit this issue to crash an affected application.
  Impact Level: Application";
tag_affected = "Pidgin version prior to 2.7.4 on Windows.";
tag_insight = "The issues are caused by errors in 'libpurple' that does not validate the return
  value from 'purple_base64_decode()' function when processing malformed Yahoo!,
  MSN, MySpaceIM, XMPP or NTLM data.";
tag_solution = "Upgrade to Pidgin version 2.7.4 or later.
...
if(description)
{
  ...
  script_name("Pidgin Libpurple 'purple_base64_decode()' Denial of Service Vulnerabilities (Win)");
  desc = "
  Summary:
  " + tag_summary + "
