...
# $Id: gb_egroupware_detect.nasl 44 2013-11-04 19:58:48Z jan $
#
...
# 
# Updated By: Antu Sanadi <santu@secpod.com> on 2011-06-01
#  - Updated to detect recent versions at line 76 
#
# Copyright:
...
tag_summary = "Detection of EGroupware.
...
if (description)
{
 ...
 script_summary("Checks for the presence of EGroupware");
...
dirs = make_list("/egroupware","/groupware", "/eGroupware/egroupware", cgi_dirs());
...
foreach dir (dirs) {
 ...
 if("<title>eGroupWare [Login]</title>" >< buf || "<title>EGroupware [Login]</title>" >< buf)
 {
     if(strlen(dir)>0) {
        install=dir;
     } else {
    ...
    log_message(data: build_detection_report(app:"EGroupware", version:vers, install:install, cpe:cpe, concluded: version[0]),
                port:port);
