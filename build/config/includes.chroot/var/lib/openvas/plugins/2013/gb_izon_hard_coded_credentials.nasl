...
# $Id: gb_izon_hard_coded_credentials.nasl 49 2013-11-07 12:55:54Z mime $
#
...
port = get_kb_item("service/telnet");
if(!port)port = 23;
...
foreach login (keys(up)) {
  ...
  if(recv =~ 'uid=[0-9]+.*gid=[0-9]+') {
    security_hole(port:port, data: 'It was possible to login with username "' + login + '" using password "' + up[login] + '"\n');
    exit(0);
  }
