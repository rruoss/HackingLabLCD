...
# $Id: secpod_cubecart_mult_xss_and_sql_inj_vuln.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_impact = "Successful exploitation will allow attacker to gain sensitive information,
  execute arbitrary scripts and execute SQL query.
  Impact Level: Application";
tag_affected = "CubeCart version 2.0.7";
...
tag_summary = "The host is running CubeCart and is prone to XSS and SQL injection
  vulnerability.";
...
if(description)
{
  ...
  script_summary("Determine if CubeCart is prone to Multiple XSS and SQL Injection Vulnerability");
...
foreach dir(make_list("/cart", "/store", "/shop", "/", cgi_dirs()))
{
  ...
  {
    ## Try SQL injection
    req = http_get (item: string(dir, "/index.php?cat_id='"), port:port);
    res = http_keepalive_send_recv(port:port,data:req);
