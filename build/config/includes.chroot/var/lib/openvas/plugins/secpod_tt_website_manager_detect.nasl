...
# $Id: secpod_tt_website_manager_detect.nasl 14 2013-10-27 12:33:37Z jan $
#
...
tag_summary = "This script finds the running TT web site manager version and
...
if(description)
{
 ...
 script_version("$Revision: 14 $");
  ...
  script_summary("Set the version of TT web site manager in KB");
...
foreach path (make_list("/", "/ttwm/tt", cgi_dirs()))
{
  sndReq = http_get(item:string(path, "/index.php"), port:ttwmPort);
  rcvRes = http_send_recv(port:ttwmPort, data:sndReq);
  if("TT Web Site Manager" >< rcvRes)
  {
