...
# $Id: secpod_ms11-015.nasl 13 2013-10-27 12:16:33Z jan $
#
...
if(description)
{
  ...
  script_summary("Check for the vulnerable 'Sbe.dll' file version");
...
## Get Version from Sbe.dll file
dllVer = fetch_file_version(sysPath, file_name:"\system32\Sbe.dll");
if(!dllVer){
  exit(0);
}
...
else if(hotfix_check_sp(winVista:3) > 0)
{
  ...
  {
    ## Windows Media Center TV Pack for Windows Vista SP1
    if(version_is_less(version:dllVer, test_version:"6.6.1000.18309")) {
      security_hole(0);
    }

    ## Check for Windows Vista SP1 Sbe.dll version
