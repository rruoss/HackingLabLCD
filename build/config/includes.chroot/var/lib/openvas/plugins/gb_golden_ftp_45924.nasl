...
# $Id: gb_golden_ftp_45924.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_summary = "Golden FTP Server is prone to a denial-of-service vulnerability.

Exploits will cause the application to crash, denying service to
legitimate users.

Golden FTP Server 4.70 is vulnerable; other versions may also
be affected.";
...
if(!egrep(pattern:"Golden FTP", string: banner))exit(0);
version = eregmatch(pattern:"Golden FTP Server ready v([0-9.]+)", string: banner);
vers = version[1];

if(!isnull(vers)) {

    if(version_is_less_equal(version:vers, test_version:"4.70")) {
