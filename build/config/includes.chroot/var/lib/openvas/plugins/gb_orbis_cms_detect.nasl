...
# $Id: gb_orbis_cms_detect.nasl 14 2013-10-27 12:33:37Z jan $
#
...
tag_summary = "This script finds the running Orbis CMS version and
...
if(description)
{
 ...
 script_version("$Revision: 14 $");
  ...
  script_summary("Set the version of Orbis CMS in KB");
...
foreach path (make_list("/orbis", "/Orbis", "/", cgi_dirs()))
{
  ## Send and Recieve the respose
  sndReq = http_get(item:string(path, "/admin/login.php"), port:orbisPort);
  rcvRes = http_send_recv(port:orbisPort, data:sndReq);

  ## Confirm it's Orbis CMS
  if(">Powered by Orbis CMS<" >< rcvRes)
  {
    sndReq = http_get(item:string(path, "/CHANGELOG.txt"), port:orbisPort);
    rcvRes = http_send_recv(port:orbisPort, data:sndReq);
