...
# $Id: gb_cutenews_n_utf8cutenews_mult_vuln.nasl 15 2013-10-27 12:49:54Z jan $
#
...
tag_solution = "For UTF-8 CuteNews Upgrade to version 8b
  ...
  For CuteNews:
  No solution or patch is available as of 10th December, 2009, Information
  regarding this issue will be updated once the solution details are available.
  For updates refer to http://cutephp.com/";

tag_impact = "Successful exploitation could allow remote attackers to steal user credentials,
  disclose file contents, disclose the file path of the application, execute
  arbitrary commands.
  Impact Level: system/Application.";
tag_affected = "CuteNews version 1.4.6 and UTF-8 CuteNews version prior to 8b";
tag_insight = "- An improper validation of user-supplied input by the 'category.db.php' script
    via the Category Access field or Icon URL fields
...
if(description)
{
  ...
  script_summary("Check for the CuteNews/UTF-8 CuteNews version");
...
if(cnVer)
{
  ...
  if(!safe_checks() && cnVer[2] != NULL)
  {
    ...
    if("<script>alert(/OpenVAS-XSS/);</script>" >< response)
    {
      security_hole(cnPort);
      exit(0);
    }
  }
