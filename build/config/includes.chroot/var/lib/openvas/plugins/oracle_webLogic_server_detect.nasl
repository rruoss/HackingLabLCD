...
# $Id: oracle_webLogic_server_detect.nasl 44 2013-11-04 19:58:48Z jan $
#
...
tag_summary = "Detection of Oracle WebLogic Server.
...
if(description)
{
 ...
 script_summary("Set KB for the version of Oracle WebLogic Server");
   ...
   egrep(pattern: "<TITLE>WebLogic Server.*Console Login", string: buf)))
{
  ...
  if(vers)
  {
    ...
    log_message(data: build_detection_report(app:"Oracle WebLogic Server", version:vers,
    install:port + '/tcp', cpe:cpe, concluded: vers), port:port);
  }
}
