...
# $Id: gb_gatesoft_docusafe_sql_inj_vuln.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_impact = "Successful exploitation will allow attacker to allow an attacker to obtain
  sensitive information.
  Impact Level: Application";
tag_affected = "GateSoft Docusafe 4.2.2 and prior";
tag_insight = "The flaw is caused by improper validation of user-supplied input via the
  'ECO_ID' parameter in 'ECO.asp', which allows attacker to manipulate
  SQL queries by injecting arbitrary SQL code.";
tag_solution = "No solution or patch is available as of 17th February, 2011. Information
  regarding this issue will be updated once the solution details are available.
  For updates refer to http://gatesoft.no/Prod_DocuSafe.asp";
tag_summary = "The host is running GateSoft Docusafe and is prone to SQL
...
if(description)
{
  ...
  script_summary("Determine if GateSoft Docusafe is prone to SQL Injection Vulnerability");
...
foreach dir(make_list("/docusafe", "/DocuSafe", "/", cgi_dirs()))
{
  ## Send and Recieve the response
  req = http_get (item: string (dir,"/main.asp"), port:port);
