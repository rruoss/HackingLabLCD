...
# $Id: gb_pivotx_detect.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_summary = "This script finds the installed PivotX version and saves
  the result in KB.";
...
if(description)
{
 ...
 script_version("$Revision: 13 $");
  ...
  script_summary("Set the version of PivotX in KB");
...
foreach path (make_list("/", "/pivotx", "/PivotX", cgi_dirs()))
{
  ## Send and receive response
  sndReq = http_get(item:string(path, "/index.php"), port:pxPort);
  rcvRes = http_send_recv(port:pxPort, data:sndReq);

  ## Confirm the application
  if(">Welcome to PivotX" >< rcvRes || ">PivotX Powered<" >< rcvRes)
  {
    ## Grep for the version
    pxVer = eregmatch(pattern:"PivotX - ([0-9.]+)", string:rcvRes);
    if(pxVer[1] != NULL)
    {
      ## Set the KB value
