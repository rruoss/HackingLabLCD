...
# $Id: gb_xdataface_dataface_lfi_vuln.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_affected = "Xataface Dataface version 1.3rc3 and prior.";
tag_insight = "The flaw is caused by improper validation of user-supplied input via the
  '-action' parameter to 'index.php',  which allows attackers to read arbitrary
  files via a ../(dot dot) sequences.";
tag_solution = "No solution or patch is available as of 09th June, 2011. Information
  regarding this issue will be updated once the solution details are available.
  For updates refer to http://xataface.com/";
tag_summary = "This host is running Xataface Dataface and is prone to local file
...
foreach dir (make_list("/Xdataface", "/dataface", "/", cgi_dirs()))
{
  ## Send and Receive the response
  req = http_get(item:string(dir,"/dataface_info.php"), port:port);
