...
# $Id: dns_xfer.nasl 17 2013-10-27 14:01:43Z jan $
...
tag_summary = "The remote name server allows DNS zone transfers to be performed.
 ...
 it to gain information about the topology of your network and spot new
 targets.";
...
if(description)
{
 ...
 summary = "Determines if the remote name server allows zone transfers";
 ...
 # should require "Services/dns" but find_service.nasl does not recognize dns
 # mayby copy copy some code from here to find_service.nasl???
...
domain = bin2string (ddata:domain);
...
if (ancount >= 1) {
  ...
  desc = string(desc,"\n\nIt was possible to initiate a zone transfer for the domain '",domain,"'\n");

  log_message(port:53,data:desc);
}
