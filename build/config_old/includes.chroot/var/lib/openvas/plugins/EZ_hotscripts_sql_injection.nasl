...
# $Id: EZ_hotscripts_sql_injection.nasl 15 2013-10-27 12:49:54Z jan $
#
...
if (description)
{
 ...
 script_summary("Determine if EZ Hotscripts is vulnerable to SQL Injection");
...
dir = make_list("/ez", cgi_dirs());

foreach d (dir)
{ 
 url = string(d, "/showcategory.php?cid=-101+union+select+12345678987654321,12345678987654321,3,4,5--");
 req = http_get(item:url, port:port);
 buf = http_keepalive_send_recv(port:port, data:req, bodyonly:0);
 if( buf == NULL )continue;

 if ( egrep(pattern:"showcategory\.php\?cid=12345678987654321.*>12345678987654321", string: buf) )
   {    
