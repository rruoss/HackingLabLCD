...
tag_insight = "A heap-based buffer overflow in the Newt library might allow remote,
    user-assisted attackers to execute arbitrary code.";
tag_solution = "All Newt users should upgrade to the latest version:

    # emerge --sync
    # emerge --ask --oneshot --verbose '>=dev-libs/newt-0.52.10-r1'

http://www.securityspace.com/smysecure/catid.html?in=GLSA%20201006-14
http://bugs.gentoo.org/show_bug.cgi?id=285854";
...
if(description)
{
 ...
 script_summary("Gentoo Security Advisory GLSA 201006-14 (newt)");
