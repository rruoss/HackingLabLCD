...
# $Id: gb_uniform_server_detect.nasl 14 2013-10-27 12:33:37Z jan $
#
...
tag_summary = "This script finds the installed Uniform Server version and saves
  the result in KB.";
...
if(description)
{
 ...
 script_version("$Revision: 14 $");
  ...
  script_summary("Set the version of Uniform Server in KB");
...
sndReq = http_get(item:"/", port:uniPort);
rcvRes = http_send_recv(port:uniPort, data:sndReq);

## Confirm the application
if(">Uniform Server" >< rcvRes )
{
  ## Grep for the version
  uniVer = eregmatch(pattern:"Uniform Server (([0-9.]+).?([a-zA-Z]+))", string:rcvRes);
  if(uniVer[1] != NULL)
  {
    ## Set the KB value
