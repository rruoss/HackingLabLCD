...
# $Id: gb_AjaXplorer_detect.nasl 44 2013-11-04 19:58:48Z jan $
#
...
tag_summary = "This host is running AjaXplorer, a file explorer
...
if (description)
{
 ...
 script_summary("Checks for the presence of AjaXplorer");
...
dirs = make_list("/filemanager",cgi_dirs());
...
foreach dir (dirs) {
 ...
 if(egrep(pattern: "AjaXplorer Version", string: buf, icase: FALSE))
 {
    ...
    if ( !isnull(version[1]) ) {
       ...
       register_host_detail(name:"App", value:string("cpe:/a:ajaxplorer:ajaxplorer:",vers), nvt:SCRIPT_OID, desc:SCRIPT_DESC);
    } else {
       register_host_detail(name:"App", value:string("cpe:/a:ajaxplorer:ajaxplorer"), nvt:SCRIPT_OID, desc:SCRIPT_DESC);
    }  
