...
# $Id: gb_eclipse_ide_mult_xss_vuln.nasl 13 2013-10-27 12:16:33Z jan $
#
...
foreach port (keys(ports))
{
  ...
  if(get_port_state(ecPort[1]))
  {
    ...
    {
      sndReq = http_get(item: "/help/advanced/content.jsp?'onload='alert" +
                                             "('XSS-Testing')", port:ecPort[1]);
      rcvRes = http_keepalive_send_recv(port:ecPort[1], data:sndReq);

      ## Check the response to confirm vulnerability
      if("alert('XSS-Testing')" >< rcvRes)
      {
