...
# $Id: gb_schneider_quantum_ethernet_module_hardcoded_credentials_ftp_51046.nasl 13 2013-10-27 12:16:33Z jan $
#
...
foreach credential (keys(credentials)) {
  ...
  if(ftp_authenticate(socket:soc, user:credential, pass:credentials[credential]) ) {
    ...
    report = string("\n\nIt was possible to login via FTP into the remote host using the following\nUsername/Password combination:\n\n",credential,":",credentials[credential],"\n\nWhich produces the following output for the 'syst' command:\n\n",result,"\n");
    desc = desc + report;
