...
# $Id: gb_wordpress_kses_library_xss_vuln.nasl 13 2013-10-27 12:16:33Z jan $
#
...
tag_affected = "WordPress versions prior to 3.0.4";
tag_insight = "The flaw is caused by input validation errors in the 'KSES HTML/XHTML' filter
...
tag_solution = "Upgrade to WordPress version 3.0.4 or later
  For updates refer to http://wordpress.org/download/";
tag_summary = "The host is running WordPress and is prone to Cross site scripting
  vulnerability.";
...
foreach dir (make_list("/wordpress", cgi_dirs()))
{
  ...
  if("Powered by WordPress" >< res)
  {
    ...
    if(res)
    {
      ...
      res = http_keepalive_send_recv(port:port, data:req);
      if(("javascript:alert('OpenVAS-XSS-Test')">< res))
      {
