...
# $Id: secpod_ms09-038.nasl 15 2013-10-27 12:49:54Z jan $
#
...
tag_impact = "Attackers can exploit this issue via maliciously crafted AVI files to cause
...
tag_insight = "This vulnerability arises due to flaws in the way Microsoft Windows handles
...
if(sysPath)
{
  dllVer = get_file_version(sysPath, file_name:"avifil32.dll");
  if(!dllVer){
    exit(0);
  }
}
...
if(sysPath)
{
  dllVer = get_file_version(sysPath, file_name:"System32\avifil32.dll");
  if(!dllVer){
    exit(0);
  }
}
...
if(hotfix_check_sp(winVista:3) > 0)
{
  ...
  {
    # Grep for avifil32.dll version < 6.0.6001.18270
  ...
  if("Service Pack 2" >< SP)
  {
    # Grep for avifil32.dll version < 6.0.6002.18049
