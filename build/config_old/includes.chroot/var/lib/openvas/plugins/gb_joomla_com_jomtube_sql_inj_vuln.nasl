...
# $Id: gb_joomla_com_jomtube_sql_inj_vuln.nasl 12 2013-10-27 11:15:33Z jan $
#
...
tag_affected = "Joomla Jomtube Component";
tag_insight = "The flaw is due to an input passed via the 'id' parameter to
  'index.php' (when 'option' is set to 'com_jomtube' and 'view' set to
  'video') is not properly sanitised before being used in an SQL query.";
...
tag_summary = "This host is running Joomla Jomtube component and is prone to SQL
...
if(description)
{
  ...
  script_summary("Check if Joomla Jomtube Component is vulnerable to SQL injection attack");
...
url = string(joomlaDir, "/index.php?option=com_jomtube&view=video&id='");

## Check the response to confirm vulnerability
if(http_vuln_check(port:joomlaPort, url:url, pattern:"Invalid argument " +
                  "supplied for foreach\(\)", extra_check:"[j|J]oomla",
                  check_header:TRUE)){
