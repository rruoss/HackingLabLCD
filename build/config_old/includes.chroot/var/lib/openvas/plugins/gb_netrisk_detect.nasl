...
# $Id: gb_netrisk_detect.nasl 15 2013-10-27 12:49:54Z jan $
#
...
tag_summary = "This script detects the installed version of NetRisk
...
if(description)
{
 ...
 script_version("$Revision: 15 $");
  ...
  script_summary("Sets the KB for the version of NetRisk");
...
foreach dir (make_list("/", "/netrisk", cgi_dirs()))
{
  sndReq = http_get(item:string(dir, "/index.php"), port:netriskPort);
  rcvRes = http_send_recv(port:netriskPort, data:sndReq);

  if(("NetRisk" >< rcvRes) && egrep(pattern:"^HTTP/.* 200 OK", string:rcvRes))
  {
    netriskVer = eregmatch(pattern:"NetRisk ([0-9.]+)", string:rcvRes);
