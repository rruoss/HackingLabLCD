...
# $Id: secpod_opera_detection_linux_900037.nasl 42 2013-11-04 19:41:32Z jan $
#
...
if(description)
{
 ...
 script_version("$Revision: 42 $");
  ...
  script_summary("Detection of installed version of Opera");
...
result = ssh_cmd(socket:sock, cmd:"uname -a");
...
foreach binaryName(operaName)
{
  ...
  if(!isnull(operaVer))
  {
    ...
    if(!isnull(cpe))
       register_product(cpe:cpe, location:binaryName, nvt:SCRIPT_OID);
