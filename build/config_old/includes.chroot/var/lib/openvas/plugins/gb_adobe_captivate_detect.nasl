...
# $Id: gb_adobe_captivate_detect.nasl 14 2013-10-27 12:33:37Z jan $
#
...
tag_summary = "This script finds the installed Adobe Captivate version and saves
  the version in KB.";
...
if(description)
{
 ...
 script_version("$Revision: 14 $");
  ...
  script_summary("Set the version of Adobe Captivate in KB");
...
## Confirm Adobe Captivate
if(!registry_key_exists(key:"SOFTWARE\Adobe\Adobe Captivate\")){
  exit(0);
}

## Get Application Installed Path
capPath = registry_get_sz(key:"SOFTWARE\Microsoft\Windows\CurrentVersion" +
                             "\App Paths\AdobeCaptivate.exe", item:"Path");
if(!capPath){
  exit(0);
}

share = ereg_replace(pattern:"([A-Z]):.*",replace:"\1$", string:capPath);
file = ereg_replace(pattern:"[A-Z]:(.*)",replace:"\1", string:capPath +
                            "\AdobeCaptivate.exe");

## Get Adobe Captivate version
capVer = GetVer(share:share, file:file);
