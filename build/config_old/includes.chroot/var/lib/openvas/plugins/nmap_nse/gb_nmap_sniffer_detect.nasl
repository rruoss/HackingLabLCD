...
# $Id: gb_nmap_sniffer_detect.nasl 10 2013-10-27 10:03:59Z jan $
#
...
tag_summary = "This script attempts to check if a target on a local Ethernet has
  its network card in promiscuous mode.

  This is a wrapper on the Nmap Security Scanner's (http://nmap.org) sniffer-detect.nse.";
...
if(description)
{
  ...
  script_summary("Checks if a target on a local Ethernet has its network card in promiscuous mode");
...
res = pread(cmd: "nmap", argv: make_list("nmap", "--script=sniffer-detect.nse",
                                         "-sn", get_host_ip()));
if(res)
{
