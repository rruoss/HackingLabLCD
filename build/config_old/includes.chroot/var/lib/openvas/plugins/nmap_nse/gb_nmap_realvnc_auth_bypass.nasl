...
# $Id: gb_nmap_realvnc_auth_bypass.nasl 10 2013-10-27 10:03:59Z jan $
#
...
tag_summary = "This script attempts to check if a VNC server is vulnerable to
...
if(description)
{
  ...
  script_summary("Checks if a VNC server is vulnerable to authentication bypass");
...
res = pread(cmd: "nmap", argv: make_list("nmap", "--script=realvnc-auth-bypass.nse",
                                         "-p", port, get_host_ip()));
if(res)
{
