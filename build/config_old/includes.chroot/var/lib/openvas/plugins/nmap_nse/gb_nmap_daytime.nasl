...
# $Id: gb_nmap_daytime.nasl 10 2013-10-27 10:03:59Z jan $
#
...
tag_summary = "This script attempts to retrieve the day and time from the Daytime service.
...
if(description)
{
  ...
  script_summary("Get the day and time from the Daytime service");
...
res = pread(cmd: "nmap", argv: make_list("nmap", "--script=daytime.nse",
                                          get_host_ip()));
if(res)
{
    ...
    if(ereg(pattern:"^\|",string:line))
    {
      ...
      if("daytime" >< result)
      {
        msg = string('Result found by Nmap Security Scanner (daytime.nse) ',
                'http://nmap.org:\n\n', result);
        security_note(data : msg, port : dtPort);
      }
    }
