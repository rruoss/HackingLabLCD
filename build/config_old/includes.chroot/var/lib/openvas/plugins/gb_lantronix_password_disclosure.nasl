...
# $Id: gb_lantronix_password_disclosure.nasl 12 2013-10-27 11:15:33Z jan $
#
...
function check_telnet(password) {
  ...
  if("Change Setup" >< recv && "Hardware:" >< recv && "Baudrate" >< recv && "Server" >< recv) {
    ...
    if(empty_password) {
      desc = desc + '\n\nIt was possible to login using an empty password\n';
    } else {
      desc = desc + '\n\nIt was possible to login using password "' + password + '"\n';
    }
    security_hole(port:telnet_port,data:desc);
    exit(0);
  }
...
function check_http(password) {
  ...
  if(buf =~ "HTTP/1.. 200") {
    ...
    if(empty_password) {
      desc = desc + '\n\nIt was possible to login as root using an empty password\n';
    } else {
      desc = desc + '\n\nIt was possible to login as root using password "' + password + '"\n';
    }
