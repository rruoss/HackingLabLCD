...
# $Id: musicd_dos.nasl 17 2013-10-27 14:01:43Z jan $
...
if ("Hello" >< recv)
{
 ...
 soc = open_sock_tcp(port);
 if(!soc) { security_warning(port:port); exit(0); }
 
 recv = recv_line(socket:soc, length: 1024, timeout: 1);

 if ("Hello" >!< recv) security_warning(port:port);
}
