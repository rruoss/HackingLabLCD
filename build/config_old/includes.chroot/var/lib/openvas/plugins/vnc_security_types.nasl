...
# $Id: vnc_security_types.nasl 17 2013-10-27 14:01:43Z jan $
# Description: VNC security types
#
# Authors:
...
tag_summary = "This script checks the remote VNC protocol version
...
if(description)
{
 ...
 script_summary( "Identifies the RFB protocol version (VNC) & security types");
...
else if (major > 3 || minor >= 7)
{
 ...
 if (n == 0)	# rejected connection
 {
  ...
  if (strlen(reason) > 0)
   ...
   security_note(port: port, data: strcat(report, 'OpenVAS could not read the reason why.'));
 }
 else
 {
