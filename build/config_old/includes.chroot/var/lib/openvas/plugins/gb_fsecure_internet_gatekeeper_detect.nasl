...
# $Id: gb_fsecure_internet_gatekeeper_detect.nasl 13 2013-10-27 12:16:33Z jan $
#
...
if("<title>F-Secure Anti-Virus Gateway for Linux</title>" >< buf)
{
    ...
    if(!isnull(j_id_jsp[1])) {
      ...
      if(!isnull(c[1])) {
        ...
        req = string(
                ...
                "Host: ", host, ":", port, "\r\n", 
	        "Referer: http://", host, ":", port,"/login.jsf\r\n",
 	  ...
 	  security_hole(port:port,data:string("The remote F-Secure Internet Gatekeeper uses default credentials (admin:admin).\nChange the default password as soon as possible\n"));
          # check for version. Only possible with default credentials

          req = string(
		      "GET /version.jsf HTTP/1.1\r\n",
		      "Host: ", host, ":", port, "\r\n",
		      "Referer: http://", host, ":", port,"/home.jsf\r\n",
		      "Cookie: JSESSIONID=",c[1],"\r\n",
		      "\r\n\r\n"
		      );
   ...
   if(vers == "unknown") {
     register_host_detail(name:"App", value:string("cpe:/a:f-secure:internet_gatekeeper:unknown::linux"), nvt:SCRIPT_OID, desc:SCRIPT_DESC);
   } else {
     register_host_detail(name:"App", value:string("cpe:/a:f-secure:internet_gatekeeper:",vers,"::linux"), nvt:SCRIPT_OID, desc:SCRIPT_DESC);
   }  


   info = string("linux\n\nF-Secure Internet Gatekeeper Version '");
   info += string(vers);
