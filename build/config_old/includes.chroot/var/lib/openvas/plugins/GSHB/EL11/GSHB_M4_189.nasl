...
#
# IT-Grundschutz, 11. EL, Maﬂnahme 4.189
#
# Authors:
...
if (! OPENVAS_VERSION)
{
        set_kb_item(name:"GSHB-11/M4_189/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-11/M4_189/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-11/M4_189/name", value:name);
...
if (WMIOSLOG == "On the Target System runs Samba, it is not an Microsoft System."){
  ...
  desc = string("Exec-Aufrufe und die Option '¸bergeordneten Pfade des\nIIS f¸r ASP Scripte' sind deaktiviert");
} else if(iisaspencmd >< "on" && iismeta >< "off"){
  result = string("nicht erf¸llt");
  desc = string("Exec-Aufrufe des IIS ist nicht deaktiviert.\nDie Option '¸bergeordneten Pfade des IIS f¸r ASP\nScripte' ist deaktiviert.");
} else if(iisaspencmd >< "off" && iismeta >< "on"){
  result = string("nicht erf¸llt");
  desc = string("Exec-Aufrufe des IIS ist deaktiviert. Die Option\n'¸bergeordneten Pfade des IIS f¸r ASP Scripte' ist\nnicht deaktiviert. Nach dem ƒndern der Option\n'¸bergeordneten Pfade des IIS f¸r ASP Scripte'\nbitte den IIS vor dem n‰chsten Test neu starten.");
} else {
  result = string("nicht erf¸llt");
  desc = string("Exec-Aufrufe des IIS ist nicht deaktiviert. Die Option\n'¸bergeordneten Pfade des IIS f¸r ASP Scripte' ist\nnicht deaktiviert. Nach dem ƒndern der Option\n'¸bergeordneten Pfade des IIS f¸r ASP Scripte'\nbitte den IIS vor dem n‰chsten Test neu starten.");
}

set_kb_item(name:"GSHB-11/M4_189/result", value:result);
set_kb_item(name:"GSHB-11/M4_189/desc", value:desc);
set_kb_item(name:"GSHB-11/M4_189/name", value:name);
