...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 5.063
#
# Authors:
...
if (SAMBA || (SSHUNAME && "command not found" >!< SSHUNAME && "CYGWIN" >!< SSHUNAME)){
  ...
  if(pubrings >< "error"){
    ...
    result = string("Fehler");
    desc = string("Vom System konnte keine Paketliste mit installierter Software geladen werden.");
  }else if(!desc1 && !desc2){
  ...
  if(GnuPGVersion >< "error"){
    result = string("Fehler");
    if (!wmilog) desc = string("Beim Testen des Systems trat ein Fehler auf, siehe Log Message!");
    if (wmilog) desc = string("Beim Testen des Systems trat ein Fehler auf: " + wmilog);
