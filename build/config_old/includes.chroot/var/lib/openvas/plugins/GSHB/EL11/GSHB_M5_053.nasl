...
#
# IT-Grundschutz, 11. EL, Maﬂnahme 5.053
#
# Authors:
...
if (! OPENVAS_VERSION)
  {
        set_kb_item(name:"GSHB-11/M5_053/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-11/M5_053/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-11/M5_053/name", value:name);
...
log = get_kb_item("GSHB/Mailbomb/log");
...
}else if(Mailbomb == "fail"){
  result = string("unvollst‰ndig");
  desc = string("Es konnte anscheinend keine Mail mit der Mailbombe verschickt\nwerden. Bitte pr¸fen Sie trotzdem das in der Scan Konfiguration\nkonfigurierte Empf‰ngerpostfach sowie den konfigurierten MTA.");
} 

set_kb_item(name:"GSHB-11/M5_053/result", value:result);
set_kb_item(name:"GSHB-11/M5_053/desc", value:desc);
set_kb_item(name:"GSHB-11/M5_053/name", value:name);
