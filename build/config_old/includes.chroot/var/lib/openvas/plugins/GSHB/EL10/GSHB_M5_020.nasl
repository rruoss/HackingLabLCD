...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 5.020
#
# Authors:
...
if(description)
{
  ...
  script_tag(name:"creation_date", value:"Tue May 04 16:04:36 2010 +0200");
...
if (! OPENVAS_VERSION)
  {
        set_kb_item(name:"GSHB-10/M5_020/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-10/M5_020/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers.");
        set_kb_item(name:"GSHB-10/M5_020/name", value:name);
...
if(OSNAME >!< "none"){
  ...
  desc = string('Es muss sichergestellt werden, dass die Dateien $HOME/.rhosts und /etc/hosts.equiv nicht\nvorhanden sind oder dass sie leer sind und der Benutzer keine Zugriffsrechte auf sie hat.');
   if (rhosts != "not found") desc += string('\nFolgende .rhost Dateien wurden gefunden:\n' + rhosts);
   if (hostsequiv != "none"){
     val = split(lshostsequiv, sep:" ", keep:0);   
     desc += string('\nFolgende Zugriffsrechte gelten f¸r -/etc/hosts.equiv- : ' + val[0] + " " + val[2] + " "+ val[3]);
   }
   if (hostsequiv != "noentry" && hostsequiv != "none")desc += string('\nFolgende Eintr‰ge wurden in  -/etc/hosts.equiv- gefunden:\n' + hostsequiv);
   
   if ("+" >< hostsequiv) desc += string('\nSollte die Benutzung der Datei -/etc/hosts.equiv- unumg‰nglich sein, muss sichergestellt sein,\ndass kein Eintrag + vorhanden ist, da hierdurch jeder Rechner vertrauensw¸rdig w¸rde.');
   
   if (rlogind != "not found" || rshd != "not found"){
     desc += string('\nEs sollte verhindert werden, dass die Daemons rlogind und rshd gestartet werden kˆnnen.\n(siehe hierzu die Datei /etc/inetd.conf und Maﬂnahme M 5.16)');
...
set_kb_item(name:"GSHB-10/M5_020/result", value:result);
set_kb_item(name:"GSHB-10/M5_020/desc", value:desc);
set_kb_item(name:"GSHB-10/M5_020/name", value:name);
