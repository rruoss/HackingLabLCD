...
tag_summary = "Reading Apache Config";
...
if(description)
{
  ...
  script_summary("Reading Apache Config (win)");
  script_category(ACT_GATHER_INFO);
  script_copyright("Copyright (c) 2009 Greenbone Networks GmbH");
  script_family("IT-Grundschutz");
  script_mandatory_keys("Compliance/Launch/GSHB");
   
  script_dependencies("secpod_reg_enum.nasl", "GSHB_WMI_Apache.nasl", "GSHB_WMI_OSInfo.nasl");
...
for(j=0; j<max_index(IncludesSplit); j++)
      {
           LogIncPath = get_kb_item("GSHB/Apache/RootPath");
           ...
           if (CustomLogInc >!< '')CustomLog = CustomLog + CustomLogInc + '|';
           ...
           if (ErrorLogInc >!< '')ErrorLog = ErrorLog + ErrorLogInc + '|';
           ...
           if (ScriptAliasInc >!< '')ScriptAlias = ScriptAlias + ScriptAliasInc + '|';
           ...
           if (DocumentRootInc >!< '')DocumentRoot = DocumentRoot + DocumentRootInc + '|';
