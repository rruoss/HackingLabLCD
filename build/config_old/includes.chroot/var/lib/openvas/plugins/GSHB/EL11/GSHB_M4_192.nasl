...
#
# IT-Grundschutz, 11. EL, Maﬂnahme 4.192
#
# Authors:
...
if (! OPENVAS_VERSION)
{
        set_kb_item(name:"GSHB-11/M4_192/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-11/M4_192/desc", value:"Leider setzen Sie einen zu alten OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-11/M4_192/name", value:name);
...
else
{
  ...
  if (NTFS >< "TRUE" && DOCF >< "TRUE" && CLOGF >< "TRUE" && ELOGF >< "TRUE")
  {
    ...
    desc = string("Auf dem System wurde der Apache Server gem‰ﬂ IT-\nGrundschutz M4.192 installiert.");
  }
  else
  {
    result = string("nicht erf¸llt");
    if (NTFS >!< "TRUE") desc = string('Das/Eines der Document Root(s) liegt nicht auf einem\nNTFS-Laufwerk!\n');
    if (DOCF >!< "TRUE") desc = desc + string('Das/Eines der Document Root(s) liegt nicht auf einer\neigenen Partition!\n');
    if (CLOGF >!< "TRUE") desc = desc + string('Das/Eines der Custom Logfile(s) liegt nicht auf einer\neigenen Partition!\n');
    if (ELOGF >!< "TRUE") desc = desc + string('Das/Eines der Error Logfile(s) liegt nicht auf einer\neigenen Partition!\n');
  }

}


set_kb_item(name:"GSHB-11/M4_192/result", value:result);
set_kb_item(name:"GSHB-11/M4_192/desc", value:desc);
set_kb_item(name:"GSHB-11/M4_192/name", value:name);
