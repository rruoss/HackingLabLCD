...
tag_summary = "IT-Grundschutz M5.091: Einsatz von Personal Firewalls für Internet-PCs.
...
if(description)
{
  ...
  if (! OPENVAS_VERSION)
  {
    ...
    script_summary("IT-Grundschutz M5.091: Einsatz von Personal Firewalls für Internet-PCs.");
...
name = 'IT-Grundschutz M5.091: Einsatz von Personal Firewalls für Internet-PCs\n';
...
if (OSVER != "none"){
  ...
  if(ruleset == "windows") {
    result = string("Fehler");
    if (OSNAME >!< "none" && OSNAME >!< "error") desc = string('Das System ist ein ' + OSNAME + ' System\n. Allerdings konnte auf das System nicht korrekt zugegriffen werden. Folgende Fehler sind aufgetreten:\n' + log);
    ...
    if(sunipfilter == "noperm" || sunipfilterstat =="noperm"){
      ...
      result = string("nicht erfüllt");
      desc = string('Eine Abfrage mit ipf -V hat ergeben, dass keine IP Filter laufen.');
    }else if (sunipfilter == "on"){
      if (sunipfilterstat == "nofilter"){
        result = string("nicht erfüllt");
        desc = string('Beim Aufruf des Befehls ipfstat -io wurden keine IP Filter zurückgegeben.');
      }else{
        result = string("unvollständig");
    ...
    result = string("unvollständig");
    if (ruleset == "notfound") desc = string("Der Befehl iptables konnte nicht gefunden werden. Wahrscheinlich reicht die Berechtigung nicht aus.");
    else if (ruleset == "noperm") desc = string("Der Befehl iptables konnte nicht ausgeführt werden, da die Berechtigung nicht ausreicht.");
    ...
    result = string("nicht erfüllt");
    desc = string('Es war nicht möglich den Befehl iptabels auszuführen und es wurden keine iptables Module im proc Filesystem gefunden.');
  }else{
    result = string("unvollständig");
    desc = string('Bitte überprüfen Sie das iptables ruleset:\n' + ruleset);  
  }
