...
#
# IT-Grundschutz, 11. EL, Maﬂnahme 5.037
#
# Authors:
...
else if (OSVER == '5.1' || (OSVER == '5.2' && OSNAME >< 'Microsoft(R) Windows(R) XP Professional x64 Edition') || (OSVER == '6.0' && OSTYPE == 1 ) || (OSVER == '6.1' && OSTYPE == 1 ))
{
    if("none" >< shares){
      ...
      desc = string("Auf dem System gibt es keine Freigaben, die f¸r ein\nPeer-to-Peer Netzwerk genutzt werden kˆnnen");
    }else
    {
      ...
      for(i=1; i<max_index(shares); i++)
      {
        if (shares[i] =~ "^[A-Za-z]\$" || shares[i] >< "ADMIN$" || shares[i] >< "IPC$")
        {
          ...
          admdesc = string('Auf dem System gibt es nur folgende Administrative Freigaben,\ndie f¸r ein Peer-to-Peer Netzwerk genutzt werden kˆnnten:\n');
        }
        else
        {
          testresult = testresult + string("FALSE");
          nonadmshare = nonadmshare + shares[i] + ";";
          nonadmdesc = string('Auf dem System gibt es folgende Freigaben, die f¸r ein\nPeer-to-Peer Netzwerk genutzt werden kˆnnten:\n');
        }
      }
...
set_kb_item(name:"GSHB-11/M5_037/result", value:result);
set_kb_item(name:"GSHB-11/M5_037/desc", value:desc);
set_kb_item(name:"GSHB-11/M5_037/name", value:name);
