...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 4.080
#
# Authors:
...
if (WMIOSLOG == "On the Target System runs Samba, it is not an Microsoft System."){
  ...
  desc = string("Das System entspricht der Maﬂnahme 4.80.");
}else{
  result = string("nicht erf¸llt");
  if (TFTP == 69)desc = string("Auf dem System l‰uft ein TFTP Server. ‹berpr¸fen Sie bitte ob die notwendig ist");
  if (FTP == 1)desc = desc + string('\n Auf dem System l‰uft ein FTP Server. ‹berpr¸fen Sie bitte ob dies notwendig ist');
  if (community)desc = desc + string('\nAuf dem System existieren folgende standardm‰ﬂig voreingestellten Community-Namen:' + SNMPHOLE);
}
