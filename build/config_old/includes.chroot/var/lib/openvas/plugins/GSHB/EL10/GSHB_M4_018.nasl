...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 4.018
#
# Authors:
...
if (! OPENVAS_VERSION)
  {
        set_kb_item(name:"GSHB-10/M4_018/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-10/M4_018/desc", value:"Leider setzen Sie einen zu alte OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-10/M4_018/name", value:name);
...
if(OSNAME >!< "none"){
  ...
  if (rcSconf != "none" && rcSconf == "noperm"){
    ...
    desc += string('Beim Testen des Systems wurde festgestellt, das Sie\nkeine Berechtigung haben die Datei /etc/init/rcS.conf\nzu lesen.'); 
      ...
      }else if("wrong:" >< rcSconf){
        rcSconf = split(rcSconf, sep:":", keep:0);
        result = string("nicht erf¸llt");
        desc = string('Folgender Eintrag wurde f¸r den Single-User-Modus in\nder Datei /etc/init/rcS.conf gefunden:\n' + rcSconf[1]);
      }else if("unknow:" >< rcSconf){
        rcSconf = split(rcSconf, sep:":", keep:0);
        result = string("nicht erf¸llt");
  ...
  if (rcSsulogin != "none" && rcSsulogin == "noperm"){
    ...
    }else if("wrong:" >< rcSsulogin){
      rcSsulogin = split(rcSsulogin, sep:":", keep:0);
      result = string("nicht erf¸llt");
      desc = string('Folgender Eintrag wurde f¸r den Single-User-Modus in\nder Datei /etc/event.d/rcS-sulogin gefunden:\n' + rcSsulogin[1]);
    }else if("unknow:" >< rcSsulogin){
      rcSsulogin = split(rcSsulogin, sep:":", keep:0);
      result = string("nicht erf¸llt");
...
set_kb_item(name:"GSHB-10/M4_018/result", value:result);
set_kb_item(name:"GSHB-10/M4_018/desc", value:desc);
set_kb_item(name:"GSHB-10/M4_018/name", value:name);
