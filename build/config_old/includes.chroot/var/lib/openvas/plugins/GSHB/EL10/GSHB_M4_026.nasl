...
#
# IT-Grundschutz, 10. EL, Maﬂnahme 4.026
#
# Authors:
...
if(description)
{
  ...
  script_tag(name:"creation_date", value:"Wed Jun 02 10:09:16 2010 +0200");
...
if (! OPENVAS_VERSION)
  {
        set_kb_item(name:"GSHB-10/M4_026/result", value:"nicht zutreffend");
        set_kb_item(name:"GSHB-10/M4_026/desc", value:"Leider setzen Sie einen zu alte OpenVAS Scan-Server ein. Bitte wechseln Sie auf die 3.0er-Serie des Scan-Servers!");
        set_kb_item(name:"GSHB-10/M4_026/name", value:name);
...
if(OSNAME >!< "none"){
  ...
  desc = string('Um das System mit SLAD zu Testen, benˆtigen Sie eine SSH Verbindung.\nFolgender Fehler ist aufgetreten:' + ssh);
}else if (sladlst == "noslad"){
  result = string("Fehler");
  desc = string('Auf dem System ist SLAD nicht installiert. Demnach kann das System nicht gem‰ﬂ Maﬂnahme 4.026 getestet werden.');
  ...
  desc += string('\nDemnach kann das System nicht gem‰ﬂ Maﬂnahme 4.026 getestet werden.');
}else{
  result = string("unvollst‰ndig");
  desc = string('Auf dem System ist Slad mit den Slad Plugins Tiger, John und Tripwire installiert.\nF¸hren Sie bitte eine OpenVAS-Pr¸fung Ihres Netzwerkes\nmit den genannten SLAD Plugins und dem aktuellen Plugin-Set aus.');
}
...
set_kb_item(name:"GSHB-10/M4_026/result", value:result);
set_kb_item(name:"GSHB-10/M4_026/desc", value:desc);
set_kb_item(name:"GSHB-10/M4_026/name", value:name);
