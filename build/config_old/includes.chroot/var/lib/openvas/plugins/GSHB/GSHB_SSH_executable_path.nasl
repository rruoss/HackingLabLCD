...
tag_summary = "List executable and writable-executable Files, list path variable over an SSH Connection.
...
if (!executable) executable = "none";
  ...
  if (Lst[i] =~ "^/usr/local/bin/.*" || Lst[i] =~ "^/usr/bin/.*" || Lst[i] =~ "^/bin/.*" || Lst[i] =~ "^/usr/games/.*" || Lst[i] =~ "^/sbin/.*" || Lst[i] =~ "^/usr/sbin/.*" ||  Lst[i] =~ "^/usr/local/sbin/.*" ||  Lst[i] =~ "^/var/lib/.*" ||  Lst[i] =~ "^/lib/.*" ||  Lst[i] =~ "^/usr/lib/.*" ||  Lst[i] =~ "^/etc/.*" ||  Lst[i] =~ "^/opt/slad/.*" ||  Lst[i] =~ ".*Keine Berechtigung.*" ||  Lst[i] =~ ".*Permission denied.*") continue;
  executable += Lst[i] + '\n';
  }
}
